...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            Divider()
            SectionTitle("Engine")
            val shizukuSub = when (state.shizukuState) {
                ShizukuManager.State.READY -> "Connected — hibernation is ready."
                ShizukuManager.State.PERMISSION_REQUIRED -> "Tap to grant Zenify access."
                ShizukuManager.State.UNAVAILABLE -> "Shizuku isn't running. Start the Shizuku app first."
            }
            Row(
                Modifier
                    .fillMaxWidth()
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text(shizukuSub, style = MaterialTheme.typography.bodySmall)
                }
                if (state.shizukuState == ShizukuManager.State.PERMISSION_REQUIRED) {
                    androidx.compose.material3.TextButton(onClick = onRequestShizuku) {
                        Text("Grant")
                    }
                }
            }
            ...
            Divider()
            Text(
                "Zenify hibernates apps with Shizuku — no root required. It force-stops " +
                    "managed apps that go idle, freeing RAM and stopping background battery drain.",
                style = MaterialTheme.typography.bodySmall,
